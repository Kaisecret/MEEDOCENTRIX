...
<style>
    ...
    .fp-current-badge { font-size:.72rem; background:#dbeafe; color:#1e40af; border-radius:999px; padding:.15rem .5rem; font-weight:700; margin-top:2px; }
...
<div class="fp-profile">
    ...
    <div class="fp-card">
        ...
        <div class="fp-card-body">
            <form action="{{ route('fishport.profile.update') }}" method="POST" id="fp-avatar-form">
                @csrf
                @method('PUT')
                <input type="hidden" name="update_mode" value="avatar">
                {{-- Hidden fields so the form is valid (other fields submitted via main form) --}}
    ...
    <div class="fp-card">
        ...
        <div class="fp-card-body">
            <form action="{{ route('fishport.profile.update') }}" method="POST">
                @csrf
                @method('PUT')
                <input type="hidden" name="update_mode" value="profile">
                {{-- Pass current avatar so it doesn't get reset --}}
                <input type="hidden" name="avatar" value="{{ $currentAvatar }}">

                <div class="fp-grid">
                    <div class="fp-field">
                        <label for="fp-name">Full Name</label>
                        <input id="fp-name" name="name" type="text" class="fp-input" value="{{ old('name', $user->name) }}" required>
                    </div>
                    <div class="fp-field">
                        <label for="fp-username">Username</label>
                        <input id="fp-username" name="username" type="text" class="fp-input" value="{{ old('username', $user->username) }}" autocomplete="off">
                    </div>
                    <div class="fp-field">
                        <label for="fp-email">Email Address</label>
                        <input id="fp-email" name="email" type="email" class="fp-input" value="{{ old('email', $user->email) }}" required>
                    </div>
                    <div class="fp-field">
