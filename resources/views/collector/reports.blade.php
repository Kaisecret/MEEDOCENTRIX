...
<div data-server-rendered-page="collector_reports" data-page-title="Collector Reports" class="cr">
    ...
    <section class="cr-card">
        <div class="cr-head"><h3>Filter Report Range</h3><div style="font-size:.82rem;color:#64748b;">{{ $rangeLabel }} • {{ $rangeStart->format('M d, Y') }} to {{ $rangeEnd->format('M d, Y') }}</div></div>
        <div class="cr-body">
    ...
    <section class="cr-grid">
        ...
        <article class="cr-kpi cr-kpi-amount"><span>Total / Accepted / Pending Amount</span><strong>PHP {{ number_format($totalAmount, 2) }}</strong><b>Accepted: PHP {{ number_format($acceptedAmount, 2) }} • Pending: PHP {{ number_format($pendingAmount, 2) }}</b></article>
    </section>
    ...
    <section class="cr-card">
        <div class="cr-head"><h3>Detailed Collection Report</h3><div class="cr-head-meta">Includes date and time per transaction</div></div>
        <div class="cr-table-wrap">
            <table class="cr-table">
                <thead>
                    <tr>
                        <th>Log ID</th><th>Payment No.</th><th>Date</th><th>Time</th><th>Vessel</th><th>ARR/DEP</th><th>Origin</th><th>Status</th><th class="cr-num">Total</th><th>Payer</th><th>Collector</th>
                    </tr>
                </thead>
                <tbody>
