...
<div data-server-rendered-page="send_payment" data-page-title="Send for Payment" class="sp-page">
    ...
    <section class="sp-hero">
        <div>
            <h1>Fishport Send for Payment</h1>
            <p>Select unpaid transactions by day/week and send in batch to assigned Fishport collector.</p>
        </div>
        <div class="sp-kpis">
    ...
    <section class="sp-card">
        <form method="GET" action="{{ route('fishport.send_payment') }}" class="sp-filter-row" id="spFilterForm">
            ...
            <input type="text" name="q" value="{{ $search }}" placeholder="Search log no, vessel, origin..." class="sp-input sp-input-search">
        </form>
    </section>

    <section class="sp-card">
        <form method="POST" action="{{ route('fishport.send_payment.store') }}" id="sendBatchForm">
            @csrf
            ...
            <div class="sp-batch-head">
                <h3><i class="fa-solid fa-list-check"></i>Unpaid Transactions Ready for Collector</h3>
            ...
            @if ($collectors->isEmpty())
                <p class="sp-hint">No collector is assigned to Fishport yet. Ask admin to assign one first.</p>
            @endif
            ...
            <div class="sp-table-wrap">
                <table class="sp-table">
                    ...
                    <tbody>
                        @forelse ($logs as $log)
                            @php
                                $queueMeta = $openDispatchByLogId[(int) $log->id] ?? null;
                                ...
                                $total = (float) ($log->paymentRecord?->total_amount ?? 0);
                            @endphp
                            <tr>
                                <td>
                                    <input
                                ...
                                <td>{{ optional($log->log_date)->format('m/d/Y') }} {{ substr((string) $log->log_time, 0, 5) }}</td>
                                ...
                                <td>
                                    @if ($queueStatus === 'sent' && $queueItemId > 0)
                                        <button
                                            ...
                                            data-log-no="{{ $log->log_number }}"
                                            data-payment-no="{{ $log->paymentRecord?->payment_number ?? '-' }}"
                                        >
                        ...
                        @empty
                            <tr><td colspan="9" class="sp-empty"><span class="sp-empty-icon"><i class="fa-solid fa-magnifying-glass"></i></span>No unpaid transactions for this filter.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div class="sp-modal">
        ...
        <div class="sp-modal-body">
            ...
            <div>
                ...
                <div class="sp-modal-meta">
                    <span id="confirmCancelLog"></span>
                    <span id="confirmCancelPayment"></span>
                </div>
            </div>
        </div>
        <div class="sp-modal-foot">
...
.sp-alert {
    ...
    width: min(400px, 100%);
    animation: spToastIn .3s ease both;
    pointer-events: auto;
    box-shadow: 0 8px 24px rgba(0,0,0,.12);
}
.sp-alert-success { background: #059669; border: 1px solid #047857; color: #fff; }
.sp-alert-error   { background: #fef2f2; border: 1px solid #fecaca; color: #991b1b; }
.sp-alert.sp-toast-exit { animation: spToastOut .3s ease forwards; }
...
.sp-input-search { min-width: 200px; }
...
.sp-batch-controls label {
    ...
    color: var(--sp-muted); text-transform: uppercase; letter-spacing: .04em;
    min-width: 240px;
}
...
.sp-send-btn:hover:not(:disabled) { background: var(--sp-primary-dk); transform: translateY(-1px); }
.sp-send-btn:disabled { opacity: .5; cursor: not-allowed; }
...
.sp-pill-green  { background: #ecfdf5; border-color: #a7f3d0; color: #065f46; }
.sp-pill-blue   { background: #eff6ff; border-color: #bfdbfe; color: #1d4ed8; }
.sp-pill-orange { background: #fffbeb; border-color: #fde68a; color: #92400e; }
...
.sp-footer-summary strong { color: var(--sp-primary); }
...
.sp-sub { color: var(--sp-muted); font-size: .83rem; }

/* Small action buttons */
.sp-btn-outline {
    display: inline-flex; align-items: center; gap: 5px;
    border: 1.5px solid var(--sp-border); background: #fff; color: var(--sp-text);
    border-radius: 7px; padding: .35rem .7rem; font-size: .8rem; font-weight: 600;
...
.sp-btn-approve {
    ...
    border-radius: 7px; padding: .35rem .75rem; font-size: .8rem; font-weight: 700;
    cursor: pointer; font-family: inherit; transition: all .15s;
}
...
/* ── Empty State ── */
.sp-empty {
    text-align: center; color: var(--sp-muted);
    padding: 2.5rem 1rem !important;
    font-size: .9rem;
}
.sp-empty-icon { font-size: 2rem; color: #cbd5e1; display: block; margin-bottom: 8px; }
...
.sp-modal-backdrop {
    ...
    padding: 1rem; z-index: 1400;
}
.sp-modal-backdrop.is-open { display: flex; }
.sp-modal {
    width: min(520px, 100%); border-radius: 16px;
...
.sp-modal-close {
    ...
    border: 1px solid var(--sp-border); background: #fff;
    color: var(--sp-text); cursor: pointer; font-size: 1.1rem;
    display: flex; align-items: center; justify-content: center;
    transition: background .15s;
}
.sp-modal-close:hover { background: #f1f5f9; }
.sp-modal-body { padding: 1.1rem 1.2rem; display: flex; gap: 12px; align-items: flex-start; }
.sp-modal-icon {
    width: 44px; height: 44px; border-radius: 12px;
    display: flex; align-items: center; justify-content: center;
    background: #dbeafe; border: 1px solid #bfdbfe; color: var(--sp-primary);
    flex-shrink: 0; font-size: 1.1rem;
}
.sp-modal-icon-warn { background: #fef3c7; border-color: #fde68a; color: var(--sp-amber); }
.sp-modal-body p { margin: 0; color: var(--sp-text); font-size: .9rem; line-height: 1.55; }
.sp-modal-meta {
    margin-top: 8px; display: flex; gap: 6px; flex-wrap: wrap;
}
.sp-modal-meta span {
    border: 1px solid var(--sp-border); background: var(--sp-soft);
    color: var(--sp-primary); border-radius: 999px;
    font-size: .76rem; font-weight: 700; padding: .2rem .6rem;
}
.sp-modal-foot {
    border-top: 1px solid var(--sp-border); padding: .9rem 1.2rem;
...
@keyframes spModalIn  { from { opacity: 0; transform: translateY(10px) scale(.97); } to { opacity: 1; transform: translateY(0) scale(1); } }

@media (max-width: 980px) {
    .sp-hero { flex-direction: column; align-items: flex-start; }
    .sp-kpis { width: 100%; grid-template-columns: repeat(3, 1fr); }
    .sp-filter-row { grid-template-columns: 1fr 1fr; }
}
@media (max-width: 640px) {
    .sp-toast-stack { top: .75rem; right: .75rem; left: .75rem; align-items: stretch; }
    .sp-alert { width: 100%; }
    .sp-filter-row { grid-template-columns: 1fr; }
    .sp-batch-controls { width: 100%; }
    .sp-batch-controls label { min-width: 100%; }
    .sp-footer-summary { justify-content: space-between; }
}
</style>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (sendBatchForm) {
            sendBatchForm.addEventListener('submit', function (event) {
                ...
                if (selected.length === 0) {
                    ...
                    alert('Select at least one transaction to send.');
                    return;
                }

                if (!collectorSelect || collectorSelect.value.trim() === '') {
                    event.preventDefault();
                ...
                const total = selected.reduce((sum, checkbox) => sum + Number(checkbox.dataset.amount || 0), 0);
                const collectorText = collectorSelect.options[collectorSelect.selectedIndex]
                    ? collectorSelect.options[collectorSelect.selectedIndex].textContent.trim()
                    : 'Selected collector';
                ...
                if (sendText) sendText.textContent = 'You are about to send selected transactions to collector queue. Continue?';
        ...
        document.querySelectorAll('.js-cancel-send-btn').forEach((button) => {
            button.addEventListener('click', function () {
                ...
                const logNo = button.getAttribute('data-log-no') || '-';
                const paymentNo = button.getAttribute('data-payment-no') || '-';

                const cancelText = document.getElementById('confirmCancelText');
                const cancelLog = document.getElementById('confirmCancelLog');
                const cancelPayment = document.getElementById('confirmCancelPayment');

                if (cancelText) cancelText.textContent = 'Are you sure you want to cancel this sent transaction from collector queue?';
                if (cancelLog) cancelLog.textContent = 'Log: ' + logNo;
                if (cancelPayment) cancelPayment.textContent = 'Payment: ' + paymentNo;
